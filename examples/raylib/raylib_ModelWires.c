...
void PF_DrawModelEx(Model model, Vector3 position, Vector3 rotationAxis, float rotationAngle, Vector3 scale, Color tint)
{
    ...
    PFmat4f matScale, matRotation, matTranslation;
    pfMat4fScale(matScale, scale.x, scale.y, scale.z);
    pfMat4fRotate(matRotation, *(PFvec3f*)(&rotationAxis), DEG2RAD(rotationAngle));
    pfMat4fTranslate(matTranslation, position.x, position.y, position.z);

    PFmat4f matTransform;
    pfMat4fMul(matTransform, matScale, matRotation);
    pfMat4fMul(matTransform, matTransform, matTranslation);

    // Combine model transformation matrix (model.transform) with matrix generated by function parameters (matTransform)
    PFmat4f modelTransform;
    pfMat4fMul(modelTransform, *(PFmat4f*)(&model.transform), matTransform);
