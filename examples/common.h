...
PFcontext PF_Init(void* pixels, PFuint width, PFuint height, PFpixelformat format, PFdatatype type);
...
PFcontext PF_Init(void* pixels, PFuint width, PFuint height, PFpixelformat format, PFdatatype type)
{
    PFcontext ctx = pfCreateContext(pixels, width, height, format, type);
    pfMakeCurrent(ctx);
    return ctx;
}
...
{
    PFfloat angleRadians = angleDegrees * PFM_DEG2RAD;
...
void PF_Begin3D(PFuint width, PFuint height, PFdouble fovy)
{
    ...
    PFfloat aspect = (PFfloat)width/(PFfloat)height;
    PFfloat top = 0.01f * tanf(fovy * 0.5f * PFM_DEG2RAD);
